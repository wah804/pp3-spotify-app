...
async function runTests() {
  ...
  try {
    ...
    // ==========================================================
    // CONFIG RELIABILITY TESTS
    // ==========================================================
    console.log('\nStarting configuration reliability test (misconfigured credentials)...');
    const configTestPort = 5006;
    const configTestServer = spawn('node', ['server.js'], {
      cwd: path.join(__dirname, '..'),
      env: { 
        ...process.env, 
        PORT: String(configTestPort),
    ...
    await new Promise((resolve) => {
      configTestServer.stdout.on('data', (data) => {
        const output = data.toString();
        if (output.includes('Server running') || output.includes('API URL')) {
    ...
    if (configServerStarted) {
      try {
        console.log(`Testing health check with misconfigured environment...`);
        const response = await fetch(`http://localhost:${configTestPort}/api/health`);
        const data = await response.json();
        
        if (response.status === 200 && data.spotifyConfigured === false) {
          console.log(`✅ Config Reliability (Misconfigured credentials) test PASSED.`);
        } else {
          console.error(`❌ Config Reliability (Misconfigured credentials) test FAILED.`);
          console.error(`Received Body:`, data);
          passed = false;
        }
      } catch (err) {
        console.error(`❌ Config Reliability (Misconfigured credentials) test FAILED with exception:`, err.message);
        passed = false;
      } finally {
        configTestServer.kill();
      }
    } else {
      console.error(`❌ Config Reliability (Misconfigured credentials) test FAILED: Server failed to start.`);
      configTestServer.kill();
      passed = false;
    }
